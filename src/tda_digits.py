...
def get_simplices(emb_graph, show=False, save=False):
    ...
    if save:
        ...
        print('Saving to ../figures/example: 5_simplices.png')
        plt.savefig('../figures/example/5_simplices.png')
...
def get_image(n=17, show=False, save=False):
    ...
    if save:
        inverted = np.array(list(map(lambda x: 255.0 - x , image)))
        plt.imshow(inverted, cmap='gray')
        print('Saving to ../figures/example: 0_original-image.png')
        plt.savefig('../figures/example/0_original-image.png')
...
def get_binary_image(image, threshold=0, show=False, save=False):
    ...
    if save:
        plt.imshow(~image.astype(bool), cmap='gray')
        print('Saving to ../figures/example: 1_binary-image.png')
        plt.savefig('../figures/example/1_binary-image.png')
...
def get_skeleton(binary_image, show=False, save=False):
    ...
    if save:
        plt.imshow(~skeleton.astype(bool), cmap='gray')
        print('Saving to ../figures/example: 2_skeleton.png')
        plt.savefig('../figures/example/2_skeleton.png')
...
def get_points(skeleton, sweep_direction='top', show=False, save=False):
    ...
    if save:
        canvas = Canvas('')
        draw_points(canvas, points)
        print('Saving to ../figures/example: 3_points.png')
        plt.savefig('../figures/example/3_points.png')
...
def draw_betti_barcodes(intervals, xlim, show=False, save=False):
    ...
    if show:
        pl.show()
    if save:
        print('Saving to ../figures/example: 6_betti-barcodes.png')
        pl.savefig('../figures/example/6_betti-barcodes.png')
...
if __name__ == '__main__':
    print('Run `../scripts/prepare_data.py` first to download the data.')
    example(save=True) # to see the example for digit 8
